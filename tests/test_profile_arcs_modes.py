"""Profile arc modes test: two modes + lines in one profile.
...
def build_part(ctx):
    p = profile(name='ArcModesP', plane='XY', at=(0, 0, 0))
    R = 5.0
    H = 10.0
    # Start at (R,0)
    p.from_(R, 0)
    ...
    p.arc(radius=R, endAt=(-R, 0), dir='cw', sweep=-90)
